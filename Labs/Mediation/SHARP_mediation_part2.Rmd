...
The first step of HIMA is to reduce the candidate mediators to a moderate number, which is less that the sample size $n$. This is achieved by "Sure Independence Screening" (SIS). Then at step two, HIMA minimized the penalized sum of squares based on the mediation model, which is
...
The output of HIMA package is a data.frame containing mediation testing results of mediators selected by the penalty terms. From left to right, the estiamtes are:
...
All exposures are significantly associated with at least one exposure selected from step 1. All of them will be included in the further analysis.
