...
# # following needed to set wd when knitting Rmarkdown (Rmd does not recognize above setwd for some reason)
# knitr::opts_knit$set(root.dir = "C:/jim/OmicsSummerWorkshop/2022/LabGxE/")
...
One can use a polytomous logistic regression model with form $$logit(Pr G=g|E, D=1) = \gamma_0 + \gamma_1 + \gamma_EE$$

or approximate this using a linear model with form $$G = \gamma_0 + \gamma_EE$$
...
model_caseonly <- lm(SNP.1 ~ e3_alcpreg_yn_None + h_age_None + h_mbmi_None +  h_cohort + h_edumc_None + e3_sex_None + ethn_PC1 + ethn_PC2, data = dataset_caseonly)
...
### Example - joint G, GxE, G|E (case+control) test (3DF)
...
## Run GxEScanR
```{r, echo = T}
# 1. read in bdinfo by using getbdinfo

# 2. read in epidata by using read.table

# 3. run GxEScanR using gweis
